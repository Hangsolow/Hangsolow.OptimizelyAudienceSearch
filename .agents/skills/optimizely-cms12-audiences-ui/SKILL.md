...
## Bulk Creation Workflow (Load Testing)

Use this when the goal is to create many audiences (for example 50) to test list performance, search, and picker filtering.

1. Define a deterministic generation pattern.
   - Name format: `AUD-<Adjective>-<Noun>-<NN>-<Token>`.
   - Notes: `Generated for addon search load test`.
   - Keep names unique and human-scannable.
2. Use Query String criterion for each generated audience.
   - Rotate `key` values across a small set such as `campaign`, `segment`, `persona`, `ab`, `source`.
   - Use `condition = Exist` for some rows and `condition = Has Value` for others.
   - When `Has Value`, set a short token value.
3. Execute in batches (recommended: 10 per batch).
4. After each batch, run verification checks before continuing.

## UI State And Refresh Guardrails

The Audiences SPA list can show stale state after successful create operations.

1. Do not trust one tab alone for final counts.
2. Re-open the Audiences list in a fresh tab and verify row count again.
3. If counts differ, use the fresh-tab count as source of truth.

## Duplicate And API Fallback

If `Duplicate` appears in the menu but does not create an entry:

1. Treat Duplicate as unsupported for the current UI build.
2. Fallback to `Create Audience` from UI using the same criterion setup.
3. If UI automation is flaky, use the same backend endpoint the UI uses (from the signed-in browser context) and then re-open the list to verify.

For API fallback in browser automation context:

1. Include anti-forgery token header (`requestverificationtoken`) on requests.
2. Clone a known-good audience payload (including criteria type and settings shape).
3. Use `POST /EPiServer/EPiServer.Cms.UI.VisitorGroups/visitorgroups/create`.
4. Confirm creation with `GET /visitorgroups/getById/{id}` and with list UI refresh.
...
For bulk runs, add these checks:

- Target count reached (for example, 50 `AUD-` audiences).
- Random sample of created audiences opens and shows Query String criterion persisted.
- Sample criteria settings (key/condition/value) persist after reload.
- Fresh-tab count matches current-tab count.
...
- `Duplicate` can no-op in some builds even when visible in the menu.
- List updates may lag in an already-open tab; validate in a fresh tab before concluding create failure.

## Query String Criterion Reference

For `Query String` criterion in this UI:

- `settings.key`: query parameter name.
- `settings.condition`: `0` = Exist, `1` = Has Value.
- `settings.value`: string value (typically null/empty when condition is Exist).

Use this mapping when validating saved criterion payloads in automation.

## Troubleshooting: "Only One Audience Was Created"

1. Open Audiences in a fresh tab and recount entries.
2. Verify no filters/search are active in the list.
3. Confirm create response succeeded (UI toast or successful API response).
4. Open one created audience and verify Query String criterion persisted.
5. If responses are successful but count still appears wrong, hard-reload and re-open list route.
